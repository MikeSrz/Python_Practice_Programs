...
while not exit:
    try:
        pais = input("Introduce el nombre de un país y te diré su idioma más hablado(Introduce 'q' para salir): ").upper()
        if pais == 'Q':
            print("Adiós :)")
            exit = True
        else:
...
while not exit:
    try:
        idioma =  input("Introduce un idioma y te mostraré todos los paises donde se habla(Introduzca 'q' para salir): ").upper()
        if idioma == 'Q':
            print('Adiós :)')
            exit = True
        else:
